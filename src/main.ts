import { everyMiute, httpRetryPolicy } from "@app/policies"
...
export function getDelay(roundTo: number, delay: number) {
  return T.effectTotal(() => {
    ...
    return calc > 60000 ? calc - 60000 : calc
  })
}
...
export const initialDelay = pipe(
  getDelay(60000, 10000),
  T.chain((delay) =>
    pipe(
      log(
        `Sleep for ${Math.round(delay / 1000)} secods until ${moment(new Date())
          .add(delay)
          .toISOString()}`
      ),
